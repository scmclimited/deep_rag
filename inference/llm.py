...
# Process the text prompt (no images for text-only chat)
import torch
...
# -------------------------
LLM_PROVIDER   = os.getenv("LLM_PROVIDER", "llava").lower()   # "openai" | "ollama" | "llava" | "gemini"
...
DEFAULT_TEMP   = float(os.getenv("LLM_TEMPERATURE", ""))

# Lazy loading for LLaVA model (only load when actually needed)
_model = None
_processor = None

# Optional: Only import clients if provider is used (avoids hard deps)
...
# Note: _list_gemini_models() removed - we use the configured model directly from .env
# No need to check available models since we know which model we're using
    ...
    for attempt in range(1, retries + 1):
        try:
            if LLM_PROVIDER == "openai":
                ...
                raise ValueError(f"Unsupported LLM_PROVIDER: {LLM_PROVIDER}. Use: 'openai', 'ollama', 'gemini', or 'llava'")
        except Exception as e:
            last_err = e
            if attempt == retries:
                break
